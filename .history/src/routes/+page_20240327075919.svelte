	...
	onMount(() => {
		...
		const hideOnScroll = () => {
			if (window.scrollY < 200 && fixed != null) {
				...
				fixed.classList.add(animate-[FileSystemFileHandle])
			} else {
...
<div
	class="duration-900 animate-[fade] duration-700 fixed bottom-24 left-0 right-0 mx-auto grid min-w-[80vw] max-w-[100vw] place-items-center items-center justify-center rounded-md bg-[#efc7c7a2] py-2 pl-4 pr-4 text-center"
>
