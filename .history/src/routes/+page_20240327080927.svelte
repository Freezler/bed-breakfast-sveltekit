    ...
    onMount(() => {
        ...
        const hideOnScroll = () => {
            if (window.scrollY < 200 && fixed != null) {
                fixed.classList.toggle('fade-out'); // Remove fade-out class if present
                fixed.classList.toggle('fade-in'); // Add fade-in class to fade in
            } else {
                if (fixed == null) return;
                fixed.classList.toggle('fade-in'); // Remove fade-in class if present
...
<style>
    ...
    .fade-in {
        animation: fade-in 0.5s; /* Adjust the duration as needed */
    }

    .fade-out {
        animation: fade-out 0.5s; /* Adjust the duration as needed */
    }
</style>